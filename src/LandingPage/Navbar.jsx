...
const Navbar = () => {
  ...
  const navItems = ['Home', 'Services', 'TechStack', 'Process', 'Projects', 'Contact'];
  ...
  const DesktopNavItem = ({ item }) => (
    ...
    >
      <span className={`text-base font-medium ${activeItem === item ? 'text-yellow-500' : 'text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-zinc-100'}`}>
        {item}
      </span>
      <span className={`absolute inset-x-0 bottom-0 h-0.5 bg-yellow-500 transform ${activeItem === item ? 'scale-x-100' : 'scale-x-0'} origin-left transition-transform duration-300 ease-in-out group-hover:scale-x-100`}></span>
    </li>
  );
  ...
  const MobileNavItem = ({ item }) => (
    <li 
      ...
      onClick={() => setActiveItem(item)}
    >
      <span className={`text-lg ${activeItem === item ? 'text-yellow-500 font-medium' : 'text-zinc-700 dark:text-zinc-300'}`}>
        {item}
      </span>
    </li>
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
        ...
        <Sheet>
          <SheetTrigger asChild className="md:hidden">
            <Button variant="ghost" size="icon" className="text-zinc-800 dark:text-zinc-200">
              <Menu className="h-6 w-6" />
            </Button>
          </SheetTrigger>
          <SheetContent side="right" className="w-72 border-l border-zinc-200 dark:border-zinc-800">
